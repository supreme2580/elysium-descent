...
pub fn spawn_dialog(
    ...
    let (left, bottom) = match config.position {
        DialogPosition::BottomCenter { bottom_margin } => (50.0 - config.width / 2.0, bottom_margin),
    };
...
pub fn animate_dialog(
    time: Res<Time>,
    mut query: Query<&mut BackgroundColor, With<Dialog>>,
) {
...
pub fn check_dialog_proximity(
    ...
    target_query: Query<(Entity, &Transform, &crate::systems::collectibles::Interactable), (With<crate::systems::collectibles::CollectibleType>, Without<crate::systems::character_controller::CharacterController>)>,
